...
const RELAY_SERVER_UDP = {
  host: "139.59.105.114", // Kontribusi atau cek relay publik disini: https://hub.docker.com/r/kelvinzer0/udp-relay
  port: 7300,
};
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname.startsWith("/gen") && url.searchParams.get("mass") !== null) {
        const fillerDomain = APP_DOMAIN;
        const uuid = crypto.randomUUID();
        const prxList = await getPrxList(PRX_BANK_URL);
        ...
        for (const prx of prxList) {
          try {
            ...
            // skip yang gagal check
          }
        }

        const result = [];
        for (const prx of activePrx) {
          const uri = new URL(`vless://${uuid}@${fillerDomain}`);
          uri.port = "443";
          uri.searchParams.set("encryption", "none");
          uri.searchParams.set("type", "ws");
          uri.searchParams.set("security", "tls");
          uri.searchParams.set("sni", fillerDomain);
          uri.searchParams.set("host", fillerDomain);
          uri.searchParams.set("path", `/${prx.prxIP}-${prx.prxPort}`);
          uri.hash = `${prx.country} ${prx.org} [${serviceName}]`;
